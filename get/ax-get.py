...
# Open Suite.
default_opensuite_src_url = "https://github.com/axelor/axelor-open-suite/archive/refs/tags/v"
default_opensuite_war_url = "https://github.com/axelor/axelor-open-suite/releases/download/v"
default_openwebapp_src_url = "https://github.com/axelor/open-suite-webapp/archive/refs/tags/v"
default_openwebapp_war_url = "https://github.com/axelor/open-suite-webapp/releases/download/v"
...
opensuite_war_url = default_opensuite_war_url + ax_ver_str + '/' + default_war_basename + ax_ver_str + ".war"
...
openwebapp_war_url = default_openwebapp_war_url + ax_ver_str + '/' + default_war_basename + ax_ver_str + ".war"
...
if is_src:
    ...
    with zipfile.ZipFile(opensuite_src_zip_name, "r") as opensuite_src_zip:
        opensuite_src_dest = os.path.join(current_path, openwebapp_src_folder_name, "modules", "axelor-open-suite")
        ...
        os.rename(os.path.join(os.path.dirname(opensuite_src_dest), opensuite_src_folder_name), opensuite_src_dest)
    os.remove(openwebapp_src_zip_name)
    os.remove(opensuite_src_zip_name)
    os.rename(openwebapp_src_folder_name, src_name)
    app_prop_path = os.path.join(src_name, "src", "main", "resources", "application.properties")
    if os.path.exists("branding_logo.png"):
        brand_path = os.path.join(src_name, "src", "main", "webapp", "img", "branding_logo.png")
        shutil.copyfile("branding_logo.png", brand_path)
        print("A personalized logo has been copied to " + brand_path + '.')
        print("Please edit the \"application.logo\" entry in " + app_prop_path + " to apply this new logo.")
    ...
    os.remove(war_name)
    app_prop_path = os.path.join(war_folder_name, "WEB-INF", "classes", "application.properties")
    if os.path.exists("branding_logo.png"):
...
print("In order to get Axelor working, the " + app_prop_path + " file needs various database-related changes.")
print()
time.sleep(PAUSE_SECONDS)
print("More specifically, the name of the database its going to use and the db user account info that owns the database needs to be entered.")
print("This script does not generate a database for you, but please ensure that no databases used for this version were used by previous versions of Axelor.")
print("This is to ensure a more reliable experience.")
print("To copy all the information from a previous instance of Axelor, please use the its built-in backup and restore feature.")
print()
time.sleep(PAUSE_SECONDS)
print("If a database hasn't been created already, it is recommended to name it after the specific version number you are running.")
print("For example, axelor-"+ ax_ver_str + " would make a great database name.")
print("This will make life easier in case it is necessary to revert to a previous version.")
print("In that case, there would be no need to worry about conflict due to a newer version of Axelor updating the database.")
if not is_src:
    time.sleep(PAUSE_SECONDS)
    print()
    print("In order to avoid having to specify " + war_folder_name + " while typing in the URL or IP Address to access Axelor, please rename the folder to \"ROOT\".")
